<?php
/**
 * Edit CTXPHC Members administration panel.
 *
 * @package WordPress
...
if ( current_user_can('edit_users') && !IS_PROFILE_PAGE )
	$submenu_file = 'ctxphc_users.php';
else
	$submenu_file = 'ctxphc_member_profile.php';

if ( current_user_can('edit_users') && !is_user_admin() )
	$parent_file = 'ctxphc_users.php';
else
	$parent_file = 'ctxphc_member_profile.php';
...
if ( is_multisite() && IS_PROFILE_PAGE && isset( $_GET[ 'newuseremail' ] ) && $current_user->ID ) {
	...
	if ( $new_email[ 'hash' ] == $_GET[ 'newuseremail' ] ) {
		...
		wp_redirect( add_query_arg( array('updated' => 'true'), self_admin_url( 'ctxphc_member_profile.php' ) ) );
		die();
	}
...
if ( ! IS_PROFILE_PAGE ) {
	if ( current_user_can( 'create_users' ) ) { ?>
		<a href="ctxphc_member_new.php" class="add-new-h2"><?php echo esc_html_x( 'Add New', 'user' ); ?></a>
	<?php } elseif ( is_multisite() && current_user_can( 'promote_users' ) ) { ?>
		<a href="ctxphc_member_new.php" class="add-new-h2"><?php echo esc_html_x( 'Add Existing', 'user' ); ?></a>
	<?php }
} ?>
</h2>

<form id="your-profile" action="<?php echo esc_url( self_admin_url( IS_PROFILE_PAGE ? 'ctxphc_member_profile.php' : 'ctxphc_member_edit.php' ) ); ?>" method="post"<?php do_action('user_edit_form_tag'); ?>>
...
<tr>
	...
	<p><?php printf( __('There is a pending change of your e-mail to <code>%1$s</code>. <a href="%2$s">Cancel</a>'), $new_email['newemail'], esc_url( self_admin_url( 'ctxphc_member_profile.php?dismiss=' . $current_user->ID . '_new_email' ) ) ); ?></p>
	</div>
	<?php endif; ?>
	</td>
</tr>
