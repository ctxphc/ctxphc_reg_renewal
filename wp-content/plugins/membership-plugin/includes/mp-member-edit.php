<?php
/**
 * Edit member administration panel.
 *
 * @package MembershipPlubin
...
wp_reset_vars( array( 'action', 'member_id', 'wp_http_referer' ) );

$member_id = (int) $member_id;
$current_user = wp_get_current_user();
if ( ! defined( 'IS_PROFILE_PAGE' ) )
	define( 'IS_PROFILE_PAGE', ( $member_id == $current_user->ID ) );

if ( ! $member_id && IS_PROFILE_PAGE )
	$member_id = $current_user->ID;
elseif ( ! $member_id && ! IS_PROFILE_PAGE )
	wp_die(__( 'Invalid user ID.' ) );
elseif ( ! get_userdata( $member_id ) )
	wp_die( __('Invalid user ID.') );
...
switch ($action) {
	case 'update':
		...
		check_admin_referer('update-user_' . $member_id);
		...
		if ( IS_PROFILE_PAGE ) {
			 ...
			 * @since 2.0.0
			 *
			 * @param int $member_id The user ID.
			 */
			do_action( 'personal_options_update', $member_id );
		} else {
			/**
			 ...
			 * @param int $member_id The user ID.
			 */
			do_action( 'edit_user_profile_update', $member_id );
		}

		if ( !is_multisite() ) {
			$errors = edit_user($member_id);
		} else {
			...
			if ( $member_id != $current_user->ID ) {
				$cap = $wpdb->get_var( "SELECT meta_value FROM {$wpdb->usermeta} WHERE user_id = '{$member_id}' AND meta_key = '{$blog_prefix}capabilities' AND meta_value = 'a:0:{}'" );
			...
			if ( $delete_role ) // stops users being added to current blog when they are edited
				delete_user_meta( $member_id, $blog_prefix . 'capabilities' );

			if ( is_multisite() && is_network_admin() && !IS_PROFILE_PAGE && current_user_can( 'manage_network_options' ) && !isset($super_admins) && empty( $_POST['super_admin'] ) == is_super_admin( $member_id ) )
				empty( $_POST['super_admin'] ) ? revoke_super_admin( $member_id ) : grant_super_admin( $member_id );
		}

		if ( !is_wp_error( $errors ) ) {
			$redirect = add_query_arg( 'updated', true, get_edit_user_link( $member_id ) );
			if ( $wp_http_referer )
	...
	default:
		$profileuser = get_user_to_edit($member_id);
	...
	<?php endif; ?>
		...
		<div class="wrap" id="profile-page">
			...
			?>>
				<?php wp_nonce_field('update-user_' . $member_id) ?>
					...
					<?php if ( count($_wp_admin_css_colors) > 1 && has_action('admin_color_scheme_picker') ) : ?>
						<tr class="user-admin-color-wrap">
							...
							<td><?php
								 ...
								 * @param int $member_id The user ID.
								 */
								do_action( 'admin_color_scheme_picker', $member_id );
								?></td>
						</tr>
					<?php
				...
				<input type="hidden" name="user_id" id="user_id" value="<?php echo esc_attr($member_id); ?>" />
