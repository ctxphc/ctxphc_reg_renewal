...
if ( isset( $_GET[ 'action' ] ) ) {
	...
	if ( isset( $_POST[ 's' ] ) ) {
		...
		cm_member_search( $search_request );
	} else {
...
function md_process_action( $member_action ) {
	...
	switch ( $member_action ) {
		...
		case 'archive':
			$report        = 'archive';
			$user_id_to_archive = esc_attr( $_GET[ 'member' ] );


			//$result = cm_update_member( $args );
			if ( $result ) {
