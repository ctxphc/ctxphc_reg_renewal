...
class EM_Gateway_Offline extends EM_Gateway {
	...
	function add_booking_form() {
        ...
        $header_button_classes = is_admin() ? 'page-title-action':'button add-new-h2';
		$back_to_button = '<a href="'.$EM_Event->get_bookings_url().'" class="'.$header_button_classes.'">'. sprintf(__('Go back to &quot;%s&quot; bookings','em-pro'), $EM_Event->name) .'</a>';
		add_action('pre_option_dbem_bookings_double',create_function('','return true;')); //so we don't get a you're already booked here message
		?>
		<div class='wrap'>
            <?php if( is_admin() ): ?><h1><?php else: ?><h2><?php endif; ?>	
			<?php echo sprintf(__('Add Booking For &quot;%s&quot;','em-pro'), $EM_Event->name) .' '. $back_to_button; ?>
