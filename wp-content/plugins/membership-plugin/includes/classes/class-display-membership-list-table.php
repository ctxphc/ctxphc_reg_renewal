...
if ( ! class_exists( 'CM_List_Table' ) ) {
	require_once( 'class-cm-list-table.php' );
}
 ...
 */
class CTXPHC_List_Table extends CM_List_Table {
	public $args;
	public $report_type;
	public $members;
	public $title;
	...
	function admin_header() {
		...
		echo '.wp-list-table .column-birthday { width: 10%;}';
	...
	function column_first_name( $item ) {
		switch ( $this->report_type ) {
			case 1;  //Active Members
				...
				break;
			case 0;  //Pending Payment Verification
				$actions = array(
	...
	function get_views() {
		...
		//All Members link
		$url_array      = array( 'page' => 'membership_reports', 'mdview' => 'all', );
		$all_url        = add_query_arg( $url_array, admin_url() );
		$class          = ( $current == 'all' ? ' class="current"' : '' );
		$views[ 'all' ] = "<a href='{$all_url}' {$class} >All Members</a>";
	...
	function months_dropdown( $table, $birthday ) {
		...
		$months = $wpdb->get_results( $wpdb->prepare( "
	        SELECT DISTINCT YEAR( birthday ) AS year, MONTH( birthday ) AS month
	        FROM %s
	        ORDER BY %s DESC
	        ", $table, 'birthdate' ) );
	...
	function usort_reorder( $a, $b ) {
		...
		$orderby = ( ! empty( $_GET[ 'orderby' ] ) ) ? $_GET[ 'orderby' ] : 'first_name'; //Defaults to first name
