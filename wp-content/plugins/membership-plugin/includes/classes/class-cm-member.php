 ...
 *
 * @return CM_Member|bool CM_Member object on success, false on failure.
 */
...
function get_member_by( $field, $value ) {
	$memberdata = CM_Member::get_cm_data_by( $field, $value );

	if ( ! $memberdata ) {
		return false;
	}

	$member = new CM_Member;
 ...
 *
 * get the current action used to call class.
 *
 * if action is missing, class_membership_processing type=blank.
 * If action is review, class_.membership_processing type=review
 * if action is update, compare to existing data and update only those fields that have changed.
 * if action is paypal or check, set status to pending.
 */
//use stdClass;
...
class CM_Member {
	 ...
	 * Constructor
	 *
	 * Retrieves the memberdata and passes it to {@link CM_Member::init()}.
	 *
	 * @since 1.1.1
	 * @access public
	 *
	 * @param int|string|stdClass|CM_Member $id Member's ID, a CM_Member object, or a member object from the DB.
	...
	public function __construct( $id = 0, $name = '' ) {
		...
		if ( $id ) {
			$data = self::get_cm_data_by( 'id', $id );
		} else {
			$data = self::get_cm_data_by( 'login', $name );
		}
	 ...
	 */
	public static function get_cm_data_by( $field, $value ) {
		global $wpdb;
