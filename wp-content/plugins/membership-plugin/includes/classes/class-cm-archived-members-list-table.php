...
/**
 * Archived Members List Table class.
 *
 * @since 3.1.0
 * @access private
 *
 ...
 */
class CM_Archived_Members_List_Table extends CM_List_Table {
	/**
	 ...
	 * @access public
	 *
	 * @see Membership_List_Table::__construct() for more information on default arguments.
	 *
	...
	public function prepare_items() {
		global $status, $membersearch;

		$membersearch = isset( $_REQUEST[ 's' ] ) ? wp_unslash( trim( $_REQUEST[ 's' ] ) ) : '';

		$status = isset( $_REQUEST[ 'status' ] ) ? $_REQUEST[ 'status' ] : '';

		$per_page         = ( $this->is_site_users ) ? 'site_users_network_per_page' : 'members_per_page';
		...
		$mp_member_search = new WP_User_Query( $args );

		$this->items = $mp_member_search->get_results();

		$this->set_pagination_args( array(
			'total_items' => $mp_member_search->get_total(),
	 ...
	 * with this table.
	 *
	 * Provides a list of status_ids and user count for that role for easy
	...
	protected function get_views() {
		...
		$status_links          = array();
		$status_links[ 'all' ] = "<a href='$url'$class>" . sprintf( _nx( 'All <span class="count">(%s)</span>', 'All <span class="count">(%s)</span>', $total_users, 'users' ), number_format_i18n( $total_users ) ) . '</a>';
		foreach ( $status_ids->get_names() as $this_role => $name ) {
			...
			$status_links[ $this_role ] = "<a href='" . esc_url( add_query_arg( 'role', $this_role, $url ) ) . "'$class>$name</a>";
		}
