...
class EM_Booking_Form {
	...
	public static function em_bookings_added($EM_Booking){
		global $wpdb;
		$EM_Form = self::get_form($EM_Booking->event_id, $EM_Booking);
		if( !empty($EM_Booking->booking_meta['registration']) && is_array($EM_Booking->booking_meta['registration']) && (!get_option('dbem_bookings_registration_disable') || $EM_Booking->person_id != get_option('dbem_bookings_registration_user')) ){
			$user_data = array();
			foreach($EM_Booking->booking_meta['registration'] as $fieldid => $field_value){
	...
	public static function event_bookings_meta_box(){
		...
		?>
		<br style="clear" />
		<h4><?php esc_html_e_emp('Booking Form','events-manager'); ?> </h4>
