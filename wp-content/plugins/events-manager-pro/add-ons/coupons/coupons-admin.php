...
class EM_Coupons_Admin {
    ...
    static function select_page() {
		...
		<div class='wrap'>
			<h1><?php _e('Edit Coupons','em-pro'); ?>
				<a href="<?php echo add_query_arg(array('action'=>'edit')); ?>" class="page-title-action"><?php esc_html_e_emp('Add New','events-manager'); ?></a>
			</h1>
			<?php echo $EM_Notices; ?>
			<form id='coupons-filter' method='post' action=''>
				...
				<div class="tablenav">			
					<div class="alignleft actions">
						<div class="subsubsub">
							<a href='<?php echo em_add_get_params($_SERVER['REQUEST_URI'], array('view'=>null, 'pno'=>null)); ?>' <?php echo ( empty($_REQUEST['view']) ) ? 'class="current"':''; ?>><?php echo sprintf( esc_html__emp( 'My %s', 'events-manager'), __('Coupons','em-pro')); ?> <span class="count">(<?php echo $coupons_mine_count; ?>)</span></a>
							<?php if( current_user_can('manage_others_bookings') ): ?>
							&nbsp;|&nbsp;
							<a href='<?php echo em_add_get_params($_SERVER['REQUEST_URI'], array('view'=>'others', 'pno'=>null)); ?>' <?php echo ( !empty($_REQUEST['view']) && $_REQUEST['view'] == 'others' ) ? 'class="current"':''; ?>><?php echo sprintf( esc_html__emp( 'All %s', 'events-manager'), __('Coupons','em-pro')); ?> <span class="count">(<?php echo $coupons_all_count; ?>)</span></a>
							<?php endif; ?>
						</div>
					</div>
					<?php
				...
				<table class='widefat'>
					<thead>
						<tr>
							<th><?php esc_html_e_emp('Name', 'events-manager') ?></th>
	...
	static function view_page(){
		...
		if( is_object($EM_Coupon) && !$EM_Coupon->can_manage('manage_bookings','manage_others_bookings') ){
			?>
			<div class="wrap"><h2><?php esc_html_e_emp('Unauthorized Access','events-manager'); ?></h2><p><?php echo sprintf(esc_html__emp('You do not have the rights to manage this %s.','events-manager'),__('coupon','em-pro')); ?></p></div>
			<?php
			return false;
		...
		<div class='wrap nosubsub'>
			...
			<div class='table-wrap'>
				<table id='dbem-bookings-table' class='widefat post '>
					<thead>
						<tr>
							<th class='manage-column' scope='col'><?php esc_html_e_emp('Event', 'events-manager'); ?></th>
							<th class='manage-column' scope='col'><?php esc_html_e_emp('Booker', 'events-manager'); ?></th>
							<th class='manage-column' scope='col'><?php esc_html_e_emp('Spaces', 'events-manager'); ?></th>
						...
						foreach($EM_Bookings as $EM_Booking){ 
							...
							<tr>
								...
								<td>										
									...
									<a class="em-bookings-edit" href="<?php echo $edit_url; ?>"><?php esc_html_e_emp('Edit/View','events-manager'); ?></a>
									<?php endif; ?>
								</td>
							</tr>
							<?php
		...
		<div class='wrap nosubsub'>
			...
			<form id='coupon-form' method='post' action=''>
				...
				<table class="form-table">
					...
					<tr valign="top">
						<th scope="row"><?php esc_html_e_emp('Name', 'events-manager') ?></th>
							<td><input type="text" name="coupon_name" value="<?php echo esc_attr($EM_Coupon->coupon_name); ?>" />
							<br />
						</td>
					</tr>
					<tr valign="top">
						<th scope="row"><?php esc_html_e_emp('Description', 'events-manager') ?></th>
					...
					<tbody class="em-date-range">
						<tr valign="top">
							<th scope="row"><?php esc_html_e_emp('Start Date', 'events-manager') ?></th>
							<td>
						...
						<tr valign="top">
							<th scope="row"><?php esc_html_e_emp('End Date', 'events-manager') ?></th>
							<td>
