...
class EM_Coupons extends EM_Object {
	...
	public static function em_booking_delete($result, $EM_Booking){
		global $wpdb;
		if($result){
			self::lower_booking_count($EM_Booking);
		}
		return $result;
	}
	...
	/**
	 * Lowers coupon use count for those belonging to this booking. Useful when deleting a booking or removing coupon association from it.
	...
	public static function lower_booking_count( $EM_Booking ){
		...
		foreach( self::booking_get_coupons($EM_Booking) as $EM_Coupon ){
			...
			if( $count ){
				//add to coupon count
				$result = $wpdb->update(EM_META_TABLE, array('meta_value'=>$count), array('object_id'=>$EM_Coupon->coupon_id, 'meta_key'=>'coupon-count'));
			}else{
				//start coupon count
				$result = $wpdb->delete(EM_META_TABLE, array('object_id'=>$EM_Coupon->coupon_id, 'meta_key'=>'coupon-count'));
			}
		}
		return apply_filters('em_coupons_lower_booking_count', $result !== false, $EM_Booking);
	}
